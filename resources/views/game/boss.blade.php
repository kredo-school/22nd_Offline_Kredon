...
.question{ position:absolute; top:10%; left:28%; width:44%; height:10%; display:flex; justify-content:center; align-items:center; text-align:center; font-size:34px; font-weight:bold; z-index:10; color:white; }
...
.a{ top:33%; } .b{ top:49%; } .c{ top:64%; } .d{ top:78%; }
...
<div class="battle-screen">
    <img src="{{ asset('images/battle-boss.png') }}" class="battle-bg">
...
<script>
 ...
 window.answer = function(index) {
        ...
        // ここでまとめて判定します
        if (enemyHP <= 0) {
            window.location.href = "{{ route('game.result') }}?status=win";
            return;
        }
        if (playerHP <= 0) {
            window.location.href = "{{ route('game.result') }}?status=lose";
            return;
        }

        // どちらもHPが残っている場合のみ、次の問題へ
        setTimeout(() => {
