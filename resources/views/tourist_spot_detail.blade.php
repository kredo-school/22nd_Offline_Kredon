...
<div style="background-color: #f8f9fa; min-height: 100vh; padding-bottom: 50px;">
    ...
    <div style="position: relative; width: 100%; height: 400px; background-color: #333;">
        @if($tourist_spot->photo_path)
            <img src="{{ asset('storage/' . $tourist_spot->photo_path) }}" alt="スポット写真" style="width: 100%; height: 100%; object-fit: cover; opacity: 0.85;">
        @else
            <div style="width: 100%; height: 100%; background-color: #666; display: flex; justify-content: center; align-items: center; color: white;">No Photo</div>
        @endif

        {{-- 上部のボタン群（戻る ＆ 保存） --}}
        <div style="position: absolute; top: 20px; right: 20px; display: flex; gap: 10px;">
            <a href="{{ route('tourist_spots.index') }}" style="background: white; color: #333; padding: 8px 16px; border-radius: 8px; text-decoration: none; font-weight: bold; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">← 戻る</a>

            <form action="{{ route('tourist_bookmarks.toggle', $tourist_spot->id) }}" method="POST" style="margin: 0;">
                @csrf
                <button type="submit" style="background: white; color: {{ $tourist_spot->isBookmarkedBy(Auth::user()) ? '#dc2626' : '#333' }}; border: none; padding: 8px 16px; border-radius: 8px; font-weight: bold; cursor: pointer; box-shadow: 0 2px 5px rgba(0,0,0,0.2);">
        ...
        {{-- 左下のタイトル＆エリア --}}
        <div style="position: absolute; bottom: 20px; left: 20px; background: rgba(0, 0, 0, 0.6); padding: 15px 25px; border-radius: 8px; color: white; max-width: 80%;">
            <h1 style="margin: 0; font-size: 28px; font-weight: bold; text-shadow: 1px 1px 3px rgba(0,0,0,0.5);">{{ $tourist_spot->name }}</h1>
            <div style="margin-top: 5px; font-size: 14px;">📍 エリア：{{ $tourist_spot->area }}</div>
        </div>
    </div>
    ...
    <div class="container" style="max-width: 1000px; margin: 30px auto; display: flex; flex-wrap: wrap; gap: 30px; padding: 0 20px;">
        ...
        <div style="flex: 2; min-width: 300px;">
            <h3 style="font-size: 20px; font-weight: bold; color: #007b8f; margin-bottom: 15px; border-bottom: 2px solid #e0e0e0; padding-bottom: 8px;">ここで体験できること</h3>
            ...
            <h3 style="font-size: 20px; font-weight: bold; color: #007b8f; margin-bottom: 15px; border-bottom: 2px solid #e0e0e0; padding-bottom: 8px;">スポット概要</h3>
            <p style="line-height: 1.8; color: #444; font-size: 15px;">
                ここにスポットの詳細な説明文が入ります。（※説明文の入力欄も後で追加しましょう！）波も穏やかで、初心者や家族連れでも安心して楽しめます。
            </p>
        </div>
        ...
        <div style="flex: 1; min-width: 250px;">
            <div style="background: white; padding: 25px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
                ...
                @if($tourist_spot->booking_url)
                    <a href="{{ $tourist_spot->booking_url }}" target="_blank" style="display: block; text-align: center; background-color: #007b8f; color: white; text-decoration: none; padding: 14px; border-radius: 8px; font-weight: bold; font-size: 15px; transition: 0.3s;">
                        予約サイトを見る
                    </a>
                @else
            ...
            @if(Auth::id() === $tourist_spot->user_id)
                <div style="margin-top: 20px; display: flex; flex-direction: column; gap: 10px;">
                    <button onclick="document.getElementById('editTouristSpotModal').classList.add('is-show')" style="background-color: #f0932b; color: white; border: none; padding: 10px; border-radius: 8px; cursor: pointer; font-weight: bold;">✏️ 情報を編集する</button>
                    <form action="{{ route('tourist_spots.destroy', $tourist_spot->id) }}" method="POST" onsubmit="return confirm('本当に削除しますか？');" style="margin: 0;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" style="width: 100%; background-color: #dc3545; color: white; border: none; padding: 10px; border-radius: 8px; cursor: pointer; font-weight: bold;">🗑️ 削除する</button>
                    </form>
                </div>
            @endif
        </div>
    ...
    {{-- 🌟 3. クチコミ（レビュー）エリア --}}
    <div class="container" style="max-width: 1000px; margin: 0 auto 30px auto; padding: 0 20px;">
        <div style="background: white; padding: 30px; border-radius: 12px; box-shadow: 0 4px 15px rgba(0,0,0,0.05);">
            <h3 style="font-size: 22px; font-weight: bold; color: #007b8f; margin-bottom: 20px; border-bottom: 2px solid #e0e0e0; padding-bottom: 10px;">クチコミ・レビュー</h3>

            {{-- ✍️ クチコミ投稿フォーム --}}
            <form action="{{ route('tourist_reviews.store', $tourist_spot->id) }}" method="POST" style="margin-bottom: 40px; background: #f8f9fa; padding: 20px; border-radius: 8px;">
                @csrf
                <h4 style="font-size: 16px; font-weight: bold; margin-bottom: 15px; color: #333;">✍️ クチコミを投稿する</h4>
                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-size: 13px; font-weight: bold; color: #555; margin-bottom: 5px;">評価（星）</label>
                    <select name="rating" required style="width: 100%; max-width: 200px; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 15px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <textarea name="comment" rows="3" placeholder="このスポットの感想やおすすめポイントを教えてください！" style="width: 100%; box-sizing: border-box; padding: 10px; border: 1px solid #ddd; border-radius: 6px; font-size: 14px; resize: vertical;"></textarea>
                </div>
                <button type="submit" style="background-color: #f0932b; color: white; border: none; padding: 10px 20px; border-radius: 6px; font-weight: bold; cursor: pointer;">投稿する</button>
            </form>

            {{-- 📋 クチコミ一覧表示 --}}
            <div>
                @if($tourist_spot->reviews->isEmpty())
                    <p style="color: #888; text-align: center; padding: 20px;">まだクチコミはありません。最初のクチコミを投稿しましょう！</p>
                @else
                    @foreach($tourist_spot->reviews()->latest()->get() as $review)
                        <div style="border-bottom: 1px solid #eee; padding-bottom: 15px; margin-bottom: 15px;">
                            ...
                            <div style="font-size: 12px; color: #888; display: flex; justify-content: space-between; align-items: center;">
                                ...
                                @if(Auth::id() === $review->user_id)
                                    <form action="{{ route('tourist_reviews.destroy', $review->id) }}" method="POST" onsubmit="return confirm('本当にこのクチコミを削除しますか？');" style="margin: 0;">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" style="background: none; border: none; color: #dc3545; font-size: 12px; cursor: pointer; text-decoration: underline;">削除</button>
                                    </form>
                                @endif
                            </div>
                        </div>
    ...
    <div class="custom-modal" id="editTouristSpotModal">
        <div class="modal-content" style="padding: 0;">
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 15px 20px; border-bottom: 1px solid #eee;">
                ...
                <button type="button" onclick="document.getElementById('editTouristSpotModal').classList.remove('is-show')" class="close-btn" style="position: static;">×</button>
            </div>
            ...
            <form action="{{ route('tourist_spots.update', $tourist_spot->id) }}" method="POST" enctype="multipart/form-data" style="padding: 20px;">
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="name" value="{{ old('name', $tourist_spot->name) }}" required style="width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #ddd; border-radius: 6px;">
                </div>
                ...
                <div style="margin-bottom: 15px;">
                    ...
                    <input type="text" name="hours" value="{{ old('hours', $tourist_spot->hours) }}" style="width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #ddd; border-radius: 6px;" placeholder="例: 10:00 - 18:00">
                </div>

                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-size: 12px; font-weight: bold; color: #555; margin-bottom: 5px;">💰 予算目安</label>
                    <input type="text" name="budget" value="{{ old('budget', $tourist_spot->budget) }}" style="width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #ddd; border-radius: 6px;">
                </div>

                <div style="margin-bottom: 15px;">
                    <label style="display: block; font-size: 12px; font-weight: bold; color: #555; margin-bottom: 5px;">🔗 予約サイトURL（https://... から入力）</label>
                    <input type="url" name="booking_url" value="{{ old('booking_url', $tourist_spot->booking_url) }}" placeholder="https://example.com" style="width: 100%; box-sizing: border-box; padding: 12px; border: 1px solid #ddd; border-radius: 6px;">
                </div>

                <div style="margin-bottom: 25px;">
                    <label style="display: block; font-size: 13px; font-weight: bold; color: #555; margin-bottom: 8px;">📸 写真を更新（変更する場合のみファイルを選択）</label>
                    <input type="file" name="photo" id="photoInput" accept="image/*" style="width: 100%;" onchange="previewImage(this)">
                    
                    <div id="imagePreviewContainer" style="margin-top: 15px; display: none;">
                        <span style="font-size: 11px; color: #666; display: block; margin-bottom: 5px;">選択された写真のプレビュー:</span>
                        <img id="photoPreview" src="" alt="プレビュー" style="max-width: 120px; max-height: 120px; object-fit: cover; border-radius: 6px; border: 1px solid #ddd;">
                    </div>
                </div>

                <div style="text-align: center;">
                    <button type="submit" style="background-color: #007b8f; color: white; border: none; padding: 14px 30px; border-radius: 25px; font-weight: bold; font-size: 15px; cursor: pointer; width: 100%;">
                        更新する
                    </button>
                </div>
            </form>
